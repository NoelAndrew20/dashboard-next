...
require("../schema/schemaAlimento.js");
const Transporte = mongoose.model("Alimento");

app.get("/getAllAlimento", async (req, res) => {
  try {
    const allAlimento = await Alimento.find({})
    .sort({ fecha: -1 }) // Cambia 'fechaRegistro' al campo apropiado de fecha en tu esquema
    .limit(30);
    res.send({ status: "ok", data: allAlimento });
  } catch (error) {
    console.log(error);
