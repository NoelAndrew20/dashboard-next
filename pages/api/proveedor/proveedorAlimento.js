...
const ProveedorAlimentoSchema = new mongoose.Schema(
    {
  ...
  {
    collection: 'proveedorAlimento',
    versionKey: false,
  }
);

const ProveedorAlimento = db.model('ProveedorAlimento', ProveedorAlimentoSchema);

app.get("/getAllProveedorAlimento", async (req, res) => {
  try {
    const solicitudes = await ProveedorAlimento.find();
