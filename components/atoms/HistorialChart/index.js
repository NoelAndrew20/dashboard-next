...
const HistorialChart = () => {
    ...
    const fetchData = async () => {
      try {
        const response = await axios.get(
          'http://192.168.100.10:3142/getAllSolicitudHistorial'
        );
  ...
  const cantidadesKg = primeros15Datos.map(
    (item) => item.etapas[0]?.cantidadKg || 0
  ); // Agregar manejo para evitar errores si etapas[0] es undefined
