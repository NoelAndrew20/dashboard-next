...
const TableGraph = ({
  ...
  const handleCheckboxChange = (event, item) => {
    ...
    const selectedFood = data.find(
      (food) => food.nombreAlimento === item.nombreAlimento
    );
  ...
  useEffect(() => {
    ...
    axios
      .get('http://192.168.100.10:3081/getAllalimentot0')
      .then((response) => {
  ...
  useEffect(() => {
    ...
    axios
      .get('http://192.168.100.10:3081/getAllalimentot1')
      .then((response) => {
  ...
  useEffect(() => {
    ...
    axios
      .get('http://192.168.100.10:3081/getAllalimentot2')
      .then((response) => {
        const jsonData = response.data;
              ...
              <li key={item.nombreAlimento}>
                <label key={index}>
                  <input
                    ...
                    value={item.nombreAlimento}
                    onChange={() => {
                      const selectedFood = complementoData.find(
                        (food) => food.nombreAlimento === item.nombreAlimento
                      );
                      setSelectedFoodData(selectedFood);
                      setShowForms((prevShowForms) => ({
                        ...prevShowForms,
                        [item.nombreAlimento]:
                          !prevShowForms[item.nombreAlimento],
                      }));
                    }}
                    checked={showForms[item.nombreAlimento]}
                  />
